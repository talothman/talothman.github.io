...
- `_layouts/` — `default.html` (base wrapper: head/meta/SEO/analytics + `{{ content }}`) is the parent of `post`, `projects`, `project`, and `about`.
- `_includes/` — `analytics.html` (Google gtag, injected only when `jekyll.environment == 'production'`) and `structured-data.html` (JSON-LD: Person + WebSite always, plus ProfilePage on `/`, CreativeWork on `layout: project`, and BlogPosting on `layout: post`).
...
- `_projects/` — a custom Jekyll collection (`output: true`). Two layouts are involved and they are easy to confuse: `_layouts/projects.html` (plural) backs `projects/index.html`, which loops `site.projects` sorted by `date` descending and renders each project's `{{ project.content }}` inline under a linked `<h2>`; `_layouts/project.html` (singular) is the per-project detail page, assigned by the collection `defaults` in `_config.yml`. Project markdown files are therefore content *fragments* that appear in both places — keep them short, since the body is duplicated between the index and the detail page.
...
- **Every project needs `title` and `description` front matter.** The detail page's `<h1>` and summary come from them, and a project missing them renders as an untitled, near-empty page. Empty collection files are worse still — a zero-byte `.md` in `_projects/` is copied verbatim into `_site` and served publicly as raw markdown rather than being skipped.
- **`projects/index.html` must keep linking each `<h2>` to `project.url`.** Without that link the detail pages are orphans, reachable only through `sitemap.xml`, with no internal link or anchor text pointing at them.
- The `size="#half"` / `size="#quarter"` attributes on project `<video>` tags are not standard HTML but *are* load-bearing — `assets/css/main.scss` selects on them (`video[size*="#half"]`) to set width. Don't strip them. They only apply inside `.projects__content`, which is why both project layouts use that wrapper class.
